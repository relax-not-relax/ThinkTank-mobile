...
class ImagesWalkthroughGame {
  ...
  Future<double> getDifficultyImagesWalkthrough(int level) async {
    // if (level == 1) {
    //   imgCount = 6;
    ...
    if (level == 1) {
      ...
      return (await getDifficultyImagesWalkthrough(level - 1) -
  ...
  Future initGame(int level, int? contestId, int? topicId) async {
    if (contestId != null) {
      ...
      time = await getDifficultyImagesWalkthrough(level) * (imgCount - 1);
    }
  }
}
